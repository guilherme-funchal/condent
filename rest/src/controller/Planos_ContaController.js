const Tabela = require('../models/planos_conta')
module.exports = {
  async create(req, res) {
    try {
      id = 0
      const { plano,categoria,descricao,pk_clinica } = req.body
      const tabela = await Tabela.findOne({ where: { id } })
      if (tabela) {
        res.status(401).json({ message: "Já existe este dado" })
      } else {
        const tabela = await Tabela.create({ plano,categoria,descricao,pk_clinica })
  ...
  async update(req, res) {
    try {
      ...
      if (!tabela) {
        ...
        const tabela = await Tabela.update({ plano,categoria,descricao,pk_clinica }, { where: { id } })
        res.status(200).json({ tabela })
      }
    } catch (error) {
      res.status(400).json({ error })

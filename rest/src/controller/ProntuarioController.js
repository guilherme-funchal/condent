const Tabela = require('../models/prontuario')
module.exports = {
  async create(req, res) {
    try {
      id = 0
      const { descricao, cod_paciente, data } = req.body
      const tabela = await Tabela.findOne({ where: { id } })
      if (tabela) {
        res.status(401).json({ message: "Já existe este dado" })
      } else {
        const tabela = await Tabela.create({ descricao, cod_paciente, data })
  ...
  async update(req, res) {
    try {
      ...
      if (!tabela) {
        ...
        const tabela = await Tabela.update({ descricao, cod_paciente, data }, { where: { id } })
        res.status(200).json({ tabela })
      }
    } catch (error) {
      res.status(400).json({ error })

const Tabela = require('../models/itens_orcamento')
module.exports = {
  async create(req, res) {
    try {
      id = 0
      const { descricao,cod_orcamento,valor } = req.body
      const tabela = await Tabela.findOne({ where: { id } })
      if (tabela) {
        res.status(401).json({ message: "Já existe este dado" })
      } else {
        const tabela = await Tabela.create({ descricao,cod_orcamento,valor })
  ...
  async update(req, res) {
    try {
      ...
      if (!tabela) {
        ...
        const tabela = await Tabela.update({ descricao,cod_orcamento,valor }, { where: { id } })
        res.status(200).json({ tabela })
      }
    } catch (error) {
      res.status(400).json({ error })
